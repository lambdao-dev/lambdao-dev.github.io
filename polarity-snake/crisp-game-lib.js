...
`,
    ...
    async function _init() {
        ...
        const viewBackground = theme.isDarkColor ? "blue" : 8;
    ...
    function drawGameOver() {
        ...
        saveCurrentColor();
        setColor(5);
        rect(17, Math.floor(size.y / 2) - 9, 66, 24);
        setColor(2);
        rect(19, Math.floor(size.y / 2) - 7, 62, 20);
        setColor(5);
        print(gameOverText, Math.floor((size.x - gameOverText.length * letterSize) / 2), Math.floor(size.y / 2) - 3, { color: 5, edgeColor: currentOptions.textEdgeColor.gameOver });
        const finalScoreText = `SCORE ${Math.floor(exports.score)}`;
        print(finalScoreText, Math.floor((size.x - finalScoreText.length * smallLetterWidth) / 2), Math.floor(size.y / 2) + 6, { color: 5, isSmallText: true, edgeColor: currentOptions.textEdgeColor.gameOver });
        loadCurrentColor();
    }
    function initRewind() {
        state = "rewind";
